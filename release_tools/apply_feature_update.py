...
def local_release_history(latest_manifest=None,limit=30):
    ...
    return out
'''
# Fix the intentionally compact helper source into valid Python before insertion.
helpers=helpers.replace('lines.extend(["",f"### {heading},"""])\n            lines[-1]=f"### {heading}"','lines.extend(["",f"### {heading}",""])')
...
'''
d=once(d,old,new,'update_check');write('dashboard.py',d)

s=read('static/settings.js');s=once(s,"renderUpdateInfo({configured:!!updateRepository,repository:updateRepository,currentVersion:state.me?.version,state:s.runtime?.updateState||{}});","renderUpdateInfo({configured:!!updateRepository,repository:updateRepository,currentVersion:state.me?.version,state:s.runtime?.updateState||{},releaseHistory:s.runtime?.releaseHistory||[]});",'settings hydration');write('static/settings.js',s)

h=read('static/index.html');h=once(h,'''<div class="update-notes hidden" id="updateNotesWrap">
...
</div>''','notes container');h=h.replace('0.5.57','0.5.58');write('static/index.html',h)

a=read('static/app.js');a=once(a,"const FRONTEND_BUILD='0.5.57';","const FRONTEND_BUILD='0.5.58';",'frontend build');start=a.find('function renderUpdateNotes(');end=a.find('async function checkForUpdates',start)
...
'''
a=a[:start]+renderer+a[end:];write('static/app.js',a)
...
data['releases'].append({'version':VERSION,'date':'2026-09-02','status':'prerelease','title':'Collapsible revision patch notes','summary':'Expanded Settings → Updates from a single latest-release note block into a collapsible patch-note history for every documented Torrent Dashboard revision.','highlights':['Settings → Updates now shows one collapsible patch-note entry per documented revision, ordered newest first.','Bundled release metadata is available immediately when the Updates page opens; a newly discovered GitHub release is merged into the history before it is installed.','The newest revision opens by default and each older revision can be expanded independently.','Future prerelease publication preserves older prereleases instead of deleting the complete prerelease history.'],'fixes':['Patch notes are no longer limited to only the latest GitHub release body.'],'technical':['The backend exposes sanitized releaseHistory metadata from release_notes/releases.json and merges the current GitHub manifest into it.','The accordion renderer continues to build DOM nodes with textContent rather than injecting release HTML.'],'validation':['Python compilation validates the release-history helper and existing update path.','JavaScript syntax validation covers the accordion, version ordering, and safe Markdown-to-DOM renderer.','Generated CHANGELOG.md and PROJECT_STATE.md must match the v0.5.58 structured metadata before publication.'],'known_issues':['Structured historical notes begin at v0.5.55 because earlier releases predate the release metadata pipeline.'],'architecture':prev.get('architecture',[])+['Update history is sourced from bundled structured release metadata and supplemented with the latest GitHub release manifest during update checks.'],'decisions':prev.get('decisions',[])+['Preserve future GitHub prereleases instead of deleting all older prereleases during publication.'],'next_steps':prev.get('next_steps',[])})
