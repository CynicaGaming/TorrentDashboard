...
'assert "body.scrollHeight" in app_js':'assert "body.scrollHeight" not in app_js',
'assert "listReserve=180" in app_js':'assert "listReserve=180" not in app_js',
'assert "--torrent-detail-expanded-height" in app_js':'assert "pane.style.removeProperty(\'--torrent-detail-expanded-height\')" in app_js',
'assert "var(--torrent-detail-expanded-height,clamp(260px,46%,420px))" in app_css':'assert "flex:0 0 clamp(260px,46vh,420px)" in app_css',
'assert "var(--torrent-detail-expanded-height,clamp(300px,46%,440px))" in app_css':'assert "flex-basis:clamp(300px,46vh,440px)" in app_css',
'assert "--torrent-workspace-height" in app_js':'assert "--torrent-list-height" in app_js and "--torrent-workspace-height" not in app_js',
...
for a,b in repls.items():
    if a not in v: raise RuntimeError(f'missing validator anchor: {a[:60]}')
    v=v.replace(a,b,1)
