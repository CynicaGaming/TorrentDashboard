...
# Version.
dashboard = load('dashboard.py')
dashboard = replace(dashboard, 'VERSION = "0.5.14"', 'VERSION = "0.5.15"', 'version')
save('dashboard.py', dashboard)

# Main HTML.
html = load('static/index.html').replace('?v=0.5.14', '?v=0.5.15')
html = sub(html, r'<button class="nav nav-root" data-view="history">Transfer History</button>', '<button class="nav nav-root" data-view="notifications">Notifications</button>', 'desktop nav')
html = sub(html, r'<button class="nav nav-root nav-parent" data-view="settings" id="settingsNavToggle"[^>]*><span>Settings</span><span class="nav-caret"[^>]*>⌄</span></button>', '<button class="nav nav-root nav-parent" data-view="settings" id="settingsNavToggle"><span>Settings</span></button>', 'settings button')
notification_view = '''<section class="view" id="view-notifications">
...
'''
html = sub(html, r'<section class="view" id="view-history">.*?(?=<section class="view" id="view-settings">)', notification_view, 'notification view', re.S)
html = sub(html, r'<nav class="mobile-nav"><button class="active" data-view="dashboard">Dashboard</button><button data-view="history">Transfer History</button><button class="admin-only" data-view="settings">Settings</button></nav>', '<nav class="mobile-nav"><button class="active" data-view="dashboard">Dashboard</button><button data-view="notifications">Notifications</button><button class="admin-only" data-view="settings">Settings</button></nav>', 'mobile nav')
...
'''
html = replace(html, '<div class="menu hidden" id="menu">', remove_modal + '<div class="menu hidden" id="menu">', 'remove modal')
save('static/index.html', html)

# Application behavior.
js = load('static/app.js')
js = replace(js, 'updateInfo:null};', 'updateInfo:null,notificationEvents:[]};', 'notification state')
js = sub(js, r"  \$\$\('\.nav-root:not\(#settingsNavToggle\),\.settings-subnav button,\.mobile-nav button'\)\.forEach\(b=>b\.addEventListener\('click',\(\)=>setView\(b\.dataset\.view\)\)\);\n  \$\('#settingsNavToggle'\)\?\.addEventListener\('click',.*?\);", "  $$('.nav-root,.settings-subnav button,.mobile-nav button').forEach(b=>b.addEventListener('click',()=>setView(b.dataset.view)));", 'settings click binding')
js = replace(js, "  $('#serverSelect').addEventListener('change',async e=>{state.server=e.target.value;state.selected.clear();await refreshStatus();if(!['all'].includes(state.server))await loadMeta()});", "  $('#serverSelect').addEventListener('change',async e=>{state.server=e.target.value;state.selected.clear();await refreshStatus();if(!['all'].includes(state.server))await loadMeta();if($('#view-notifications')?.classList.contains('active'))renderNotifications()});", 'server select')
js = replace(js, "  $('#historyRange').addEventListener('change',loadHistory);", "  $('#notificationFilter')?.addEventListener('change',renderNotifications);$('#refreshNotifications')?.addEventListener('click',loadNotifications);", 'notification controls')
js = replace(js, "  $('#addBtn').addEventListener('click',()=>$('#addModal').classList.remove('hidden'));$$('[data-modalclose]').forEach(x=>x.addEventListener('click',()=>$('#addModal').classList.add('hidden')));$('#addForm').addEventListener('submit',addTorrent);", "  $('#addBtn').addEventListener('click',()=>$('#addModal').classList.remove('hidden'));$$('[data-modalclose]').forEach(x=>x.addEventListener('click',()=>$('#addModal').classList.add('hidden')));$('#addForm').addEventListener('submit',addTorrent);$('#removeForm')?.addEventListener('submit',e=>{e.preventDefault();closeRemoveDialog({deleteFiles:!!$('#removeFiles')?.checked})});$$('[data-remove-cancel]').forEach(x=>x.addEventListener('click',()=>closeRemoveDialog(null)));", 'remove dialog bindings')
js = replace(js, "  window.addEventListener('keydown',e=>{if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)){e.preventDefault();$('#search').focus()}if(e.key==='Escape'){if(state.selected.size){state.selected.clear();render();return}closeDrawer();$('#addModal').classList.add('hidden')}});", "  window.addEventListener('keydown',e=>{if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)){e.preventDefault();$('#search').focus()}if(e.key==='Escape'){if(!$('#removeModal')?.classList.contains('hidden')){closeRemoveDialog(null);return}if(state.selected.size){state.selected.clear();render();return}closeDrawer();$('#addModal').classList.add('hidden')}});", 'escape handling')
js = sub(js, r"function setSettingsNavExpanded\(expanded\)\{.*?\}\nfunction setView\(view\)\{.*?\}\n\nasync function loadServers", "function setSettingsNavExpanded(expanded){const group=$('#settingsNavGroup'),submenu=$('#settingsSubnav');if(!group||!submenu)return;group.classList.toggle('expanded',!!expanded);submenu.classList.toggle('hidden',!expanded)}\nfunction setView(view){if(view==='settings'&&!state.me?.can_manage){view='dashboard';toast('Administrator Access Is Required','error')}const settingsView=view==='settings';$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('.nav-root,.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));setSettingsNavExpanded(settingsView);$('#pageTitle').textContent=uiText(view);$('#subtitle').textContent=uiText(view==='dashboard'?'liveTorrentActivity':view==='notifications'?'recentDashboardActivity':'dashboardConfiguration');if(view==='notifications')loadNotifications();if(settingsView){TDSettings.activate(localStorage.tdSettingsPage||'general');loadSettings().then(()=>TDSettings.loadExtras())}}\n\nasync function loadServers", 'settings view behavior', re.S)
js = sub(js, r"\n    if\(a==='delete'\)\{.*?\n    \}\n    if\(a==='force_start'\)", "\n    if(a==='delete')return removeTorrentTargets([{server:sid,hash:h,name:t.name||h}]);\n    if(a==='force_start')", 'context delete', re.S)
js = sub(js, r"async function bulkAction\(a\)\{.*?\}\n\nasync function loadMeta", "async function bulkAction(a){if(a==='delete'){const targets=[...state.selected].map(k=>{let [sid,...rest]=k.split(':');const hash=rest.join(':');const t=state.torrents.find(x=>(x._server_id||state.server)===sid&&x.hash===hash);return{server:sid,hash,name:t?.name||hash}});const removed=await removeTorrentTargets(targets);if(removed){state.selected.clear();render()}return}let grouped={};for(const k of state.selected){let [sid,...rest]=k.split(':');(grouped[sid]??=[]).push(rest.join(':'))}for(const [sid,hashes]of Object.entries(grouped))await doAction(a,{server:sid,hashes});state.selected.clear();render()}\n\nasync function loadMeta", 'bulk delete', re.S)
js = sub(js, r"if\(a==='delete'\)\{let files=confirm\('Delete downloaded files too\?'\);if\(confirm\('Confirm torrent deletion\?'\)\)\{await doAction\(a,\{server,hashes:\[hash\],delete_files:files\}\);closeDrawer\(\)\}return\}", "if(a==='delete'){const t=state.torrents.find(x=>(x._server_id||state.server)===server&&x.hash===hash);if(await removeTorrentTargets([{server,hash,name:t?.name||hash}]))closeDrawer();return}", 'detail delete')
remove_helpers = r'''
...
'''
js = replace(js, 'async function doAction(action,payload={})', remove_helpers + 'async function doAction(action,payload={})', 'remove helpers')
...
js = sub(js, r"async function loadHistory\(\)\{.*?\n\nfunction renderServerSettings", notification_functions + '\n\nfunction renderServerSettings', 'history JS', re.S)
save('static/app.js', js)

# Styling.
css = load('static/app.css')
css += r'''
...
'''
save('static/app.css', css)

# Service worker revisions.
sw = load('static/sw.js')
sw = replace(sw, 'torrent-dashboard-v0514', 'torrent-dashboard-v0515', 'cache version').replace('0.5.14', '0.5.15')
save('static/sw.js', sw)

# Release regression checks.
validator = load('release_tools/validate_ui_strings.py')
...
checks = """    assert '0.5.14 settings de-duplication' in settings_css
...
"""
validator = replace(validator, marker, checks, 'validator checks')
save('release_tools/validate_ui_strings.py', validator)

print('Staged Torrent Dashboard 0.5.15')
