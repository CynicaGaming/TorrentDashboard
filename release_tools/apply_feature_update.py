...
# Windows network discovery is invoked while opening settings. When the dashboard
# was restarted detached by the updater, ipconfig could briefly allocate its own
# console window. Run that background process with CREATE_NO_WINDOW instead.
old_detect = '''def _detect_windows_interfaces():
    try:
        ...
        return []
'''
new_detect = '''def _windows_background_process_kwargs():
    if os.name != "nt":
        return {}
...
def _detect_windows_interfaces():
    try:
        ...
        return []
'''
assert old_detect in dashboard
dashboard = dashboard.replace(old_detect, new_detect, 1)
write("dashboard.py", dashboard)

# Settings HTML: one Test notification action; no standalone permission action.
html = read("static/index.html")
html = html.replace(OLD, NEW)
old_actions = '<div class="settings-inline-actions notification-actions"><button class="secondary" id="previewSound" type="button">Preview Sound</button><button class="secondary" id="settingsNotifyPermission" type="button">Request Browser Permission</button></div>'
new_actions = '<div class="settings-inline-actions notification-actions"><button class="secondary" id="testNotification" type="button">Test Notification</button></div>'
...
assert 'settingsNotifyPermission' not in html and 'id="notifyPermission"' not in html
write("static/index.html", html)

# Application notification helper supports service-worker notifications where
# available, which behaves better for installed/mobile PWAs than new Notification.
...
insert = needle + "async function showBrowserNotification(title,options={}){if(!('Notification'in window))throw new Error('Browser notifications are not supported');if(Notification.permission!=='granted')throw new Error('Browser notification permission is not granted');if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.ready;if(reg?.showNotification){await reg.showNotification(title,options);return}}catch{}}new Notification(title,options)}\n"
assert needle in app
app = app.replace(needle, insert, 1)
old_completion = "if(state.settings?.notifications?.browser&&'Notification' in window&&Notification.permission==='granted')new Notification('torrentCompleted',{body:t.name})"
new_completion = "if(state.settings?.notifications?.browser&&'Notification' in window&&Notification.permission==='granted')showBrowserNotification(state.settings?.dashboard?.title||'Torrent Dashboard',{body:`Completed: ${t.name}`,tag:`torrent-complete-${k}`}).catch(()=>{})"
assert old_completion in app
app = app.replace(old_completion, new_completion, 1)
...
assert "#notifyPermission" not in app
write("static/app.js", app)

# Notifications test now requests permission as part of the user gesture and
# tests whichever notification channels are currently enabled in the form.
...
start = settings.index("  async function previewNotificationSound() {")
end = settings.index("  function updateSourceRepository() {", start)
new_notification_test = '''  async function testNotification() {
    ...
    if (soundEnabled) {
      if (mode === 'custom' && file) {
        ...
        soundUrl = `/api/notification-sound?ts=${Date.now()}`;
      } else {
        soundUrl = `/static/default-completion.wav?v=${encodeURIComponent(state.me?.version || '')}`;
      }
    }
    ...
    try {
      ...
      if (browserEnabled) {
        ...
        if (permission !== 'granted') {
          throw new Error(permission === 'denied' ? 'Browser notification permission is blocked. Enable it in this site\'s browser permissions.' : 'Browser notification permission was not granted.');
        }
      ...
      if (revoke) URL.revokeObjectURL(soundUrl);
    }
  }

'''
settings = settings[:start] + new_notification_test + settings[end:]
assert 'previewNotificationSound' not in settings
assert 'requestBrowserNotificationPermission' not in settings
assert 'settingsNotifyPermission' not in settings

# Required markers need to live in the same inline label row instead of becoming
# their own CSS-grid row. Also make the User Group control explicitly full width.
replacements = {
