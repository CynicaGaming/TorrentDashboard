...
function renderAddFiles(files){const body=$('#addFileRows');if(!files.length){body.innerHTML='<tr><td colspan="4"><div class="add-content-empty">Metadata does not contain a file list.</div></td></tr>';return}body.innerHTML=files.map((file,index)=>{const filePath=String(file.path||file.name||`File ${index+1}`),depth=Math.max(0,filePath.split(/[\\/]/).length-1),priority=Number.isFinite(Number(file.priority))?Number(file.priority):1,checked=priority!==0;return`<tr data-add-file-index="${index}"><td class="add-file-check"><input class="add-file-enabled" type="checkbox" ${checked?'checked':''}></td><td><div class="add-file-name"><span class="file-depth" style="width:${depth*12}px"></span><span>${esc(filePath)}</span></div></td><td class="mono">${bytes(file.length||0)}</td><td><select class="add-file-priority" ${checked?'':'disabled'}><option value="1" ${priority===1?'selected':''}>Normal</option><option value="6" ${priority===6?'selected':''}>High</option><option value="7" ${priority===7?'selected':''}>Maximum</option></select></td></tr>`}).join('');body.querySelectorAll('.add-file-enabled').forEach(check=>check.addEventListener('change',()=>{check.closest('tr').querySelector('.add-file-priority').disabled=!check.checked}))}
...
function bindUI(){if(bound)return;bound=true;
  ...
  $('#serverSelect').addEventListener('change',async e=>{state.server=e.target.value;state.selected.clear();closeDetailPane();await refreshStatus();if(!['all'].includes(state.server))await loadMeta();if($('#view-notifications')?.classList.contains('active'))renderNotifications()});
