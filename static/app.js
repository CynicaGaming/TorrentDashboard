...
const embeddedConsoleState={history:[],historyIndex:0,initialized:false};
function appendEmbeddedConsole(text,tone='muted'){const output=$('#embeddedConsoleOutput');if(!output)return;const line=document.createElement('div');line.className=`embedded-console-line ${tone}`;line.textContent=String(text??'');output.appendChild(line);output.scrollTop=output.scrollHeight}
function initializeEmbeddedConsole(){if(embeddedConsoleState.initialized)return;embeddedConsoleState.initialized=true;const role=state.me?.group_label||uiText(state.me?.group||'standard');const who=state.me?.display_name||state.me?.username||'User';if($('#embeddedConsoleSession'))$('#embeddedConsoleSession').textContent=`${who} · ${role}`;appendEmbeddedConsole(`Torrent Dashboard Console v${state.me?.version||FRONTEND_BUILD}`,'good');appendEmbeddedConsole(`Authorized as ${who} · ${role}. Type help to list commands available to your role.`,'muted')}
async function clearEmbeddedFrontendCache(){appendEmbeddedConsole('td> frontend clear-cache','command');try{if('serviceWorker'in navigator){const registrations=await navigator.serviceWorker.getRegistrations();await Promise.all(registrations.map(reg=>reg.unregister()))}if('caches'in window){const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith('torrent-dashboard-')).map(key=>caches.delete(key)))}appendEmbeddedConsole('Frontend service workers and Torrent Dashboard caches cleared.','good')}catch(error){appendEmbeddedConsole(error?.message||String(error),'bad')}}
async function runEmbeddedConsole(raw){const command=String(raw||'').trim();if(!command)return;embeddedConsoleState.history.push(command);if(embeddedConsoleState.history.length>100)embeddedConsoleState.history=embeddedConsoleState.history.slice(-100);embeddedConsoleState.historyIndex=embeddedConsoleState.history.length;if(command==='clear'){$('#embeddedConsoleOutput').textContent='';return}if(command==='reload'){appendEmbeddedConsole('td> reload','command');location.reload();return}if(command==='frontend clear-cache'){await clearEmbeddedFrontendCache();return}appendEmbeddedConsole(`td> ${command}`,'command');try{const data=await post('/api/recovery/command',{command});if(data.output)appendEmbeddedConsole(data.output,data.state==='installing'?'good':'muted');if(data.state==='installing')appendEmbeddedConsole('The dashboard may disconnect while the verified update is installed.','good')}catch(error){appendEmbeddedConsole(error?.message||String(error),'bad')}}
function bindEmbeddedConsole(){const form=$('#embeddedConsoleForm'),input=$('#embeddedConsoleCommand');if(!form||form.dataset.bound==='1')return;form.dataset.bound='1';form.addEventListener('submit',event=>{event.preventDefault();const command=input.value;input.value='';runEmbeddedConsole(command)});$('#embeddedConsoleHelp')?.addEventListener('click',()=>runEmbeddedConsole('help'));$('#embeddedConsoleClear')?.addEventListener('click',()=>{$('#embeddedConsoleOutput').textContent='';input.focus()});input.addEventListener('keydown',event=>{if(event.key==='ArrowUp'){event.preventDefault();if(embeddedConsoleState.history.length){embeddedConsoleState.historyIndex=Math.max(0,embeddedConsoleState.historyIndex-1);input.value=embeddedConsoleState.history[embeddedConsoleState.historyIndex]||''}}else if(event.key==='ArrowDown'){event.preventDefault();embeddedConsoleState.historyIndex=Math.min(embeddedConsoleState.history.length,embeddedConsoleState.historyIndex+1);input.value=embeddedConsoleState.historyIndex<embeddedConsoleState.history.length?embeddedConsoleState.history[embeddedConsoleState.historyIndex]:''}else if(event.key.toLowerCase()==='l'&&event.ctrlKey){event.preventDefault();$('#embeddedConsoleOutput').textContent=''}})}
...
function bindUI(){if(bound)return;
  ...
  bindAddTorrentUI();bindEmbeddedConsole();$('#removeForm')?.addEventListener('submit',e=>{e.preventDefault();closeRemoveDialog({deleteFiles:!!$('#removeFiles')?.checked})});$$('[data-remove-cancel]').forEach(x=>x.addEventListener('click',()=>closeRemoveDialog(null)));
  ...
  $('#accountSettingsBtn').addEventListener('click',()=>{hideAccountMenu();openAccountModal('profile')});$('#accountConsoleBtn')?.addEventListener('click',()=>{hideAccountMenu();setView('console')});$('#logoutBtn').addEventListener('click',()=>{hideAccountMenu();signOut()});$$('[data-account-close]').forEach(x=>x.addEventListener('click',closeAccountModal));$('#accountProfileForm').addEventListener('submit',saveOwnProfile);$('#accountPasswordForm').addEventListener('submit',changeOwnPassword);$('#accountRecoveryKeyBtn')?.addEventListener('click',regenerateOwnRecoveryKey);$('#accountRecoveryKeyCopy')?.addEventListener('click',copyOwnRecoveryKey);$('#accountChooseAvatar').addEventListener('click',()=>$('#accountAvatarInput').click());$('#accountAvatarInput').addEventListener('change',uploadOwnAvatar);$('#accountRemoveAvatar').addEventListener('click',removeOwnAvatar);bindPasswordConfirmation();
  ...
  window.addEventListener('keydown',e=>{if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)){e.preventDefault();$('#search').focus()}if(e.key==='Escape'){if(!$('#notificationBellPanel')?.classList.contains('hidden')){setNotificationBellOpen(false);return}if(!$('#passwordConfirmModal')?.classList.contains('hidden')){closePasswordConfirmation(null);return}if(!$('#clientSettingsModal')?.classList.contains('hidden')){TDSettings.closeClientSettings();return}if(!$('#accountModal')?.classList.contains('hidden')){closeAccountModal();return}if(!$('#accountMenu')?.classList.contains('hidden')){hideAccountMenu();return}if(!$('#actionDialogModal')?.classList.contains('hidden')){closeActionDialog(null);return}if(!$('#removeModal')?.classList.contains('hidden')){closeRemoveDialog(null);return}if(!$('#addModal')?.classList.contains('hidden')){closeAddTorrent();return}if(state.selected.size){state.selected.clear();render();return}if(state.detailExpanded){state.detailExpanded=false;syncDetailDock()}}});
...
function setView(view){if(view==='settings'&&!state.me?.can_manage){view='dashboard';toast('Administrator access is required','error')}const settingsView=view==='settings',dashboardView=view==='dashboard',consoleView=view==='console';$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('.nav-root,.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));setSettingsNavExpanded(settingsView);$('#pageTitle').textContent=uiText(view);$('#subtitle').textContent=view==='dashboard'?uiText('liveTorrentActivity'):view==='notifications'?uiText('recentDashboardActivity'):consoleView?'Recovery and diagnostic commands':uiText('dashboardConfiguration');if(dashboardView)requestAnimationFrame(()=>{syncTorrentWorkspaceLayout();syncDesktopDetailPaneHeight()});if(view==='notifications')loadNotifications();if(consoleView){initializeEmbeddedConsole();setTimeout(()=>$('#embeddedConsoleCommand')?.focus(),0)}if(settingsView){TDSettings.activate(localStorage.tdSettingsPage||'general');loadSettings().then(()=>TDSettings.loadExtras())}}
...
const NOTIFICATION_BELL_LIMIT=8;
...
function renderAccountRecoveryKey(user={}){const stateEl=$('#accountRecoveryKeyState'),meta=$('#accountRecoveryKeyMeta'),button=$('#accountRecoveryKeyBtn');if(!stateEl||!meta||!button)return;const configured=!!user.recovery_key_configured;stateEl.textContent=configured?'Recovery key configured':'Not configured';const created=Number(user.recovery_key_created_at||0);const ending=String(user.recovery_key_last4||'');meta.textContent=configured?`${created?`Created ${when(created)} · `:''}${ending?`Ending ${ending}`:'Recovery access enabled'}`:'Generate a recovery key to enable key-based recovery access.';button.textContent=configured?'Regenerate recovery key':'Generate recovery key';button.disabled=!user.password_configured}
function clearAccountRecoveryKeyReveal(){const wrap=$('#accountRecoveryKeyReveal'),value=$('#accountRecoveryKeyValue');if(wrap)wrap.classList.add('hidden');if(value)value.textContent=''}
async function regenerateOwnRecoveryKey(){const status=$('#accountStatus');if(!accountProfileSnapshot?.password_configured){status.className='test-result bad';status.textContent='Set an account password before generating a recovery key.';return}const password=await requestPasswordConfirmation('Confirm your current password to generate a new recovery key. The previous key will stop working immediately.');if(password===null)return;status.className='test-result muted';status.textContent=accountProfileSnapshot?.recovery_key_configured?'Regenerating recovery key…':'Generating recovery key…';try{const data=await post('/api/account/recovery-key',{current_password:password});accountProfileSnapshot={...data.user};applyAccountUser(data.user);renderAccountRecoveryKey(data.user);const wrap=$('#accountRecoveryKeyReveal'),value=$('#accountRecoveryKeyValue');if(value)value.textContent=data.recovery_key||'';wrap?.classList.remove('hidden');status.className='test-result ok';status.textContent='Recovery key generated. Store it before closing this window.';}catch(error){status.className='test-result bad';status.textContent=error.message}}
async function copyOwnRecoveryKey(){const value=$('#accountRecoveryKeyValue')?.textContent||'';if(!value)return;try{await navigator.clipboard.writeText(value);toast('Recovery key copied')}catch{toast('Could not copy recovery key','error')}}
...
async function loadAccount(){
  ...
  renderAccountRecoveryKey(d.user);clearAccountRecoveryKeyReveal();
  return d.user;
}
...
function closeAccountModal(){$('#accountModal').classList.add('hidden');$('#accountProfileForm')?.reset();$('#accountPasswordForm')?.reset();clearAccountRecoveryKeyReveal();$('#accountStatus').textContent='';accountProfileSnapshot=null}
