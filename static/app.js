...
async function installUpdate(){const version=state.updateInfo?.state?.version||state.settings?.runtime?.updateState?.version||$('#updateLatest').textContent;if(!confirm(`${uiText('installAndRestart')} ${version}?`))return;const b=$('#updateAction');if(b){b.disabled=true;b.textContent=uiText('restarting…')}try{await post('/api/update-install',{version});$('#updateMessage').textContent=`${uiText('installing')} ${version} · ${uiText('torrentDashboardWillRestart')}`;$('#updateState').textContent=uiText('installing');toast('installingUpdate');waitForUpdatedServer(version)}catch(e){if(b){b.disabled=false;b.textContent=uiText('installUpdate')}toast(e.message,'error')}}
...
function maybeAutoCheckUpdates(){const u=state.settings?.updates;if(!u?.enabled||!u?.auto_check)return;const key='tdLastUpdateCheck',hours=Math.max(1,Number(u.check_hours||6)),last=Number(localStorage[key]||0);if(Date.now()-last<hours*3600000)return;localStorage[key]=String(Date.now());checkForUpdates(true).then(d=>{if(d?.updateAvailable)toast(`updateAvailable ${d.manifest.version}`)}).catch(()=>{})}
...
function checkCompletions(){const now=new Set(state.torrents.filter(t=>Number(t.progress)>=.999999).map(keyFor));if(state.lastComplete.size){for(const k of now)if(!state.lastComplete.has(k)){const t=state.torrents.find(x=>keyFor(x)===k);if(t){toast(`completed: ${t.name}`);playCompletionSound().catch(()=>{});if(state.settings?.notifications?.browser&&'Notification' in window&&Notification.permission==='granted')new Notification('torrentCompleted',{body:t.name})}}}state.lastComplete=now;if('setAppBadge'in navigator){let n=state.torrents.filter(isActive).length;n?navigator.setAppBadge(n):navigator.clearAppBadge()}}
...
async function detailAction(a){if(!a||!state.detail)return;const {server,hash}=state.detail;if(a==='set_location'){let location=prompt('New save location:');if(location!==null)await doAction(a,{server,hashes:[hash],location});return}if(a==='rename'){let name=prompt('New torrent name:');if(name)await doAction(a,{server,hash,name});return}if(a==='set_category'){let category=prompt('Category name:');if(category!==null)await doAction(a,{server,hashes:[hash],category});return}if(a==='add_tags'){let tags=prompt('Comma-separated tags:');if(tags)await doAction(a,{server,hashes:[hash],tags});return}if(a==='set_download_limit'||a==='set_upload_limit'){let kb=prompt('Limit KB Per Second (0 = Unlimited):','0');if(kb!==null)await doAction(a,{server,hashes:[hash],limit:Number(kb)*1024});return}if(a==='delete'){let files=confirm('Delete downloaded files too?');if(confirm('Confirm torrent deletion?')){await doAction(a,{server,hashes:[hash],delete_files:files});closeDrawer()}return}await doAction(a,{server,hashes:[hash]})}
...
function addServerRow(s={id:'',name:'',base_url:'http://127.0.0.1:8080',auth_method:'api_key',api_key:'',username:'',password:'',enabled:true}){
  ...
  d.querySelector('[data-k="auth_method"]').addEventListener('change',sync);sync();d.querySelector('.danger').onclick=()=>d.remove();d.querySelector('.test-server').onclick=()=>testServerRow(d);$('#serverSettings').append(d);applySentenceCaseUi(d);decorateSecretFields(d)
}
