...
async function registerAccount(event){event.preventDefault();const error=$('#registerError'),status=$('#registerStatus'),button=$('#registerForm button[type="submit"]');if(error)error.textContent='';if(status){status.className='test-result muted';status.textContent=''}const password=$('#registerPass')?.value||'';
  if(password!==password2){const el=$('#registerError');if(el)el.textContent='Passwords do not match';return;}
  const password2=$('#registerPass2')?.value||'';if(password.length<8){if(error)error.textContent='Password must be at least 8 characters';return}if(button)button.disabled=true;try{const data=await rawJson('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('#registerUser')?.value.trim()||'',password,password2})});$('#registerForm')?.reset();if(status){status.className='test-result ok';status.textContent=data.message||'Registration submitted. Your account is Pending.'}}catch(err){if(error)error.textContent=err.message||'Registration could not be submitted'}finally{if(button)button.disabled=false}}
...
function setView(view){if(view==='settings'&&!state.me?.can_manage){view='dashboard';toast('Administrator access is required','error')}const settingsView=view==='settings',dashboardView=view==='dashboard';$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('.nav-root,.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));setSettingsNavExpanded(settingsView);$('#pageTitle').textContent=uiText(view);$('#subtitle').textContent=view==='dashboard'?uiText('liveTorrentActivity'):view==='notifications'?uiText('recentDashboardActivity'):uiText('dashboardConfiguration');if(dashboardView)requestAnimationFrame(()=>{syncTorrentWorkspaceLayout();syncDesktopDetailPaneHeight()});if(view==='notifications')loadNotifications();if(settingsView){TDSettings.activate(localStorage.tdSettingsPage||'general');loadSettings().then(()=>TDSettings.loadExtras())}}
