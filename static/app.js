...
function initializeEmbeddedConsole(){if(embeddedConsoleState.initialized)return;embeddedConsoleState.initialized=true;appendEmbeddedConsole(`Torrent Dashboard Console v${state.me?.version||FRONTEND_BUILD}`,'good');appendEmbeddedConsole('Authorized with your dashboard administrator session. Type help to list available commands.','muted')}
...
function setView(view){if((view==='settings'||view==='console')&&!state.me?.can_manage){view='dashboard';toast('Administrator access is required','error')}const settingsView=view==='settings',dashboardView=view==='dashboard',consoleView=view==='console';$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${view}`));$$('.nav-root,.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));setSettingsNavExpanded(settingsView);$('#pageTitle').textContent=uiText(view);$('#subtitle').textContent=view==='dashboard'?uiText('liveTorrentActivity'):view==='notifications'?uiText('recentDashboardActivity'):consoleView?'Administrative recovery commands':uiText('dashboardConfiguration');if(dashboardView)requestAnimationFrame(()=>{syncTorrentWorkspaceLayout();syncDesktopDetailPaneHeight()});if(view==='notifications')loadNotifications();if(consoleView){initializeEmbeddedConsole();setTimeout(()=>$('#embeddedConsoleCommand')?.focus(),0)}if(settingsView){TDSettings.activate(localStorage.tdSettingsPage||'general');loadSettings().then(()=>TDSettings.loadExtras())}}
