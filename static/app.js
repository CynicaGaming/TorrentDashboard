...
function configuredCompletionSoundUrl(){const n=state.settings?.notifications||state.me?.notifications||{};return n.sound_mode==='custom'&&(n.custom_sound_file||n.custom_sound_configured)?`/api/notification-sound?ts=${Date.now()}`:`/static/default-completion.wav?v=${encodeURIComponent(state.me?.version||'')}`}
...
async function bootstrap(){
  ...
  try{
    ...
    syncCurrentUserUi();
    if(state.me.can_manage){await loadSettings()}else{state.settings={dashboard:{low_disk_gb:20},notifications:state.me.notifications||{browser:false,sound:false,rules:{}}}}
...
function closeActionDialog(result=null){const modal=$('#actionDialogModal');hideSurface(modal);const resolve=actionDialogResolve;actionDialogResolve=null;if(resolve)resolve(result)}
...
function renderServerSettings(servers){$('#serverSettings').innerHTML='';servers.forEach(s=>addServerRow(s,false));updateServerHealthCards(Object.values(state.clientHealth||{}))}
...
function closePasswordConfirmation(result=null){const modal=$('#passwordConfirmModal');hideSurface(modal);const input=$('#passwordConfirmInput');if(input){input.value='';input.type='password';syncSecretToggle(input)}const status=$('#passwordConfirmStatus');if(status)status.textContent='';const resolve=passwordConfirmationResolve;passwordConfirmationResolve=null;if(resolve)resolve(result)}
...
async function signOut(){const names=dirtyScopeNames();if(names.length&&!await confirmDiscardScopes(names))return;try{await post('/api/logout',{})}catch{}location.reload()}
