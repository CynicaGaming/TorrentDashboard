...
let addMetadataTimer=null,addMetadataGeneration=0,addMetadataState={mode:'link',source:'',metadata:null,supported:true,pending:false,parsedFile:false};
function clearAddMetadataTimer(){if(addMetadataTimer){clearTimeout(addMetadataTimer);addMetadataTimer=null}}
function resetAddMetadata(clearSource=false){clearAddMetadataTimer();addMetadataGeneration++;addMetadataState={mode:addMetadataState.mode||'link',source:'',metadata:null,supported:true,pending:false,parsedFile:false};$('#addMetadataProgress')?.classList.add('hidden');const status=$('#addMetadataStatus');if(status){status.textContent='Waiting for metadata.';status.parentElement.className='add-metadata-status'}$('#addContentSummary').textContent='Waiting for a torrent source.';$('#addFileRows').innerHTML='<tr><td colspan="4"><div class="add-content-empty">Choose a torrent source to retrieve metadata.</div></td></tr>';for(const [id,val] of [['addInfoSize','—'],['addInfoDate','—'],['addInfoHashV1','—'],['addInfoHashV2','—'],['addInfoCreator','—'],['addInfoComment','—']])$(('#'+id)).textContent=val;if(clearSource){$('#addUrls').value='';$('#torrentFile').value=''}syncAddTorrentSubmit()}
function setAddMetadataStatus(text,tone=''){const el=$('#addMetadataStatus');el.textContent=text;el.parentElement.className='add-metadata-status'+(tone?` ${tone}`:'');$('#addMetadataProgress').classList.toggle('hidden',tone!=='loading')}
function addSingleSource(){const list=$('#addUrls').value.split(/\n/).map(x=>x.trim()).filter(Boolean);return list.length===1?list[0]:''}
function metadataTotalSize(m){const info=m?.info||{};if(Number.isFinite(Number(info.length)))return Number(info.length);return(info.files||[]).reduce((sum,f)=>sum+Number(f.length||0),0)}
function renderAddMetadata(metadata){metadata=metadata||{};addMetadataState.metadata=metadata;const info=metadata.info||{};const files=Array.isArray(info.files)?info.files:[];const total=metadataTotalSize(metadata);$('#addInfoSize').textContent=total?bytes(total):'—';$('#addInfoDate').textContent=Number(metadata.creation_date)>1?when(metadata.creation_date):'—';$('#addInfoHashV1').textContent=metadata.infohash_v1||'N/A';$('#addInfoHashV2').textContent=metadata.infohash_v2||'N/A';$('#addInfoCreator').textContent=metadata.created_by||'—';$('#addInfoComment').textContent=metadata.comment||'—';if(info.name&&!$('#addRename').value)$('#addRename').placeholder=info.name;renderAddFiles(files);if(info.name)$('#addContentSummary').textContent=`${files.length} file${files.length===1?'':'s'} · ${bytes(total)}`;else if(files.length)$('#addContentSummary').textContent=`${files.length} file${files.length===1?'':'s'} · ${bytes(total)}`;syncAddTorrentSubmit()}
function renderAddFiles(files){const body=$('#addFileRows');if(!files.length){body.innerHTML='<tr><td colspan="4"><div class="add-content-empty">Metadata does not contain a file list.</div></td></tr>';return}body.innerHTML=files.map((f,i)=>{const path=String(f.path||f.name||`File ${i+1}`),depth=Math.max(0,path.split(/[\\/]/).length-1),priority=Number.isFinite(Number(f.priority))?Number(f.priority):1,checked=priority!==0;return`<tr data-add-file-index="${i}"><td class="add-file-check"><input class="add-file-enabled" type="checkbox" ${checked?'checked':''}></td><td><div class="add-file-name"><span class="file-depth" style="width:${depth*12}px"></span><span title="${esc(path)}">${esc(path)}</span></div></td><td class="mono">${bytes(f.length||0)}</td><td><select class="add-file-priority" ${checked?'':'disabled'}><option value="1" ${priority===1?'selected':''}>Normal</option><option value="6" ${priority===6?'selected':''}>High</option><option value="7" ${priority===7?'selected':''}>Maximum</option></select></td></tr>`}).join('');body.querySelectorAll('.add-file-enabled').forEach(c=>c.addEventListener('change',()=>{const row=c.closest('tr');row.querySelector('.add-file-priority').disabled=!c.checked}));}
...
function syncAddTorrentSubmit(){const btn=$('#addTorrentSubmit');if(!btn)return;const sourceCount=addMetadataState.mode==='file'?($('#torrentFile').files.length?1:0):$('#addUrls').value.split(/\n/).map(x=>x.trim()).filter(Boolean).length;btn.disabled=!sourceCount||(addMetadataState.supported&&addMetadataState.pending)}
async function fetchAddMetadata(generation=addMetadataGeneration){const source=addSingleSource();if(!source||generation!==addMetadataGeneration)return;if(addMetadataState.source!==source){addMetadataState.source=source;addMetadataState.metadata=null}addMetadataState.pending=true;setAddMetadataStatus('Retrieving metadata…','loading');$('#addContentSummary').textContent='Retrieving torrent metadata…';syncAddTorrentSubmit();try{const d=await post('/api/torrent-metadata/fetch',{server:state.server,source});if(generation!==addMetadataGeneration)return;addMetadataState.supported=d.supported!==false;if(d.metadata&&Object.keys(d.metadata).length)renderAddMetadata(d.metadata);if(d.supported===false){addMetadataState.pending=false;setAddMetadataStatus(d.error||'Metadata preview is unavailable for this qBitTorrent version.','bad');$('#addContentSummary').textContent='The torrent can still be added without a preview.';syncAddTorrentSubmit();return}if(d.pending){addMetadataState.pending=true;addMetadataTimer=setTimeout(()=>fetchAddMetadata(generation),1000);return}addMetadataState.pending=false;setAddMetadataStatus('Metadata retrieval complete','ok');syncAddTorrentSubmit()}catch(e){if(generation!==addMetadataGeneration)return;addMetadataState.pending=false;setAddMetadataStatus(e.message,'bad');$('#addContentSummary').textContent='Metadata retrieval failed.';syncAddTorrentSubmit()}}
function scheduleAddMetadata(){clearAddMetadataTimer();addMetadataGeneration++;const generation=addMetadataGeneration;addMetadataState.source='';addMetadataState.metadata=null;addMetadataState.supported=true;addMetadataState.pending=false;const list=$('#addUrls').value.split(/\n/).map(x=>x.trim()).filter(Boolean);if(!list.length){resetAddMetadata(false);return}if(list.length>1){resetAddMetadata(false);$('#addContentSummary').textContent='Metadata preview is available when adding one torrent at a time.';setAddMetadataStatus('Multiple torrent sources selected.');syncAddTorrentSubmit();return}addMetadataTimer=setTimeout(()=>fetchAddMetadata(generation),450);syncAddTorrentSubmit()}
async function parseAddTorrentFile(){clearAddMetadataTimer();addMetadataGeneration++;const generation=addMetadataGeneration,file=$('#torrentFile').files[0];addMetadataState.source='';addMetadataState.metadata=null;addMetadataState.pending=false;addMetadataState.supported=true;addMetadataState.parsedFile=false;if(!file){resetAddMetadata(false);return}addMetadataState.pending=true;setAddMetadataStatus('Retrieving metadata…','loading');$('#addContentSummary').textContent='Parsing torrent metadata…';syncAddTorrentSubmit();try{const fd=new FormData();fd.append('server',state.server);fd.append('torrent',file,file.name);const d=await api('/api/torrent-metadata/parse',{method:'POST',body:fd});if(generation!==addMetadataGeneration)return;addMetadataState.supported=d.supported!==false;if(d.supported===false){setAddMetadataStatus(d.error||'Metadata preview is unavailable for this qBitTorrent version.','bad');$('#addContentSummary').textContent='The torrent can still be added without a preview.';syncAddTorrentSubmit();return}addMetadataState.source=d.source||'';addMetadataState.parsedFile=!!d.source;renderAddMetadata(d.metadata||{});setAddMetadataStatus('Metadata retrieval complete','ok')}catch(e){if(generation!==addMetadataGeneration)return;setAddMetadataStatus(e.message,'bad');$('#addContentSummary').textContent='Metadata retrieval failed.'}finally{addMetadataState.pending=false;syncAddTorrentSubmit()}}
...
function bindUI(){if(bound)return;bound=true;
  ...
  $('#addLinkBtn').addEventListener('click',()=>openAddTorrent('link'));$('#addFileBtn').addEventListener('click',()=>openAddTorrent('file'));$$('[data-modalclose]').forEach(x=>x.addEventListener('click',closeAddTorrent));$('#addForm').addEventListener('submit',addTorrent);$('#addUrls').addEventListener('input',scheduleAddMetadata);$('#torrentFile').addEventListener('change',parseAddTorrentFile);$('#addAutoTmm').addEventListener('change',e=>{$('#addPath').disabled=e.target.value==='automatic';$('#addUseDownloadPath').disabled=e.target.value==='automatic'});$('#addUseDownloadPath').addEventListener('change',e=>$('#addDownloadPathWrap').classList.toggle('hidden',!e.target.checked));$('#addSelectAll').addEventListener('click',()=>{$$('#addFileRows .add-file-enabled').forEach(x=>{x.checked=true;x.closest('tr').querySelector('.add-file-priority').disabled=false})});$('#addSelectNone').addEventListener('click',()=>{$$('#addFileRows .add-file-enabled').forEach(x=>{x.checked=false;x.closest('tr').querySelector('.add-file-priority').disabled=true})});$('#removeForm')?.addEventListener('submit',e=>{e.preventDefault();closeRemoveDialog({deleteFiles:!!$('#removeFiles')?.checked})});$$('[data-remove-cancel]').forEach(x=>x.addEventListener('click',()=>closeRemoveDialog(null)));
...
async function openDetail(server,hash){const same=state.detail?.server===server&&state.detail?.hash===hash;state.detail={server,hash,data:same?state.detail?.data:null};state.detailTab=state.detailTab||'general';$('#torrentDetailPane').classList.remove('hidden');$$('[data-detailtab]').forEach(b=>b.classList.toggle('active',b.dataset.detailtab===state.detailTab));const t=state.torrents.find(x=>(x._server_id||state.server)===server&&x.hash===hash);$('#detailName').textContent=t?.name||hash;$('#detailMeta').textContent=`${t?._server_name||server} · ${hash}`;render();await refreshDetailData(true)}
function closeDetailPane(){$('#torrentDetailPane').classList.add('hidden');state.detail=null;render()}
async function refreshDetailData(force=false){if(!state.detail)return;const now=Date.now();if(!force&&now-detailRefreshAt<3000)return;detailRefreshAt=now;const {server,hash}=state.detail;if(!state.detail.data)$('#detailBody').innerHTML='<div class="empty">Loading…</div>';try{const data=await api(`/api/detail?server=${encodeURIComponent(server)}&hash=${encodeURIComponent(hash)}`);if(!state.detail||state.detail.server!==server||state.detail.hash!==hash)return;state.detail.data=data;renderDetail()}catch(e){if(state.detail)$('#detailBody').innerHTML=`<div class="banner error">${esc(e.message)}</div>`}}
...
async function addTorrent(e){e.preventDefault();if(state.server==='all')return toast('chooseSpecificServerFirst','error');const file=$('#torrentFile').files[0],source=addMetadataState.mode==='file'?(addMetadataState.parsedFile?addMetadataState.source:''):$('#addUrls').value.trim();const base={server:state.server,savepath:$('#addPath').value.trim(),download_path:$('#addDownloadPath').value.trim(),use_download_path:$('#addUseDownloadPath').checked,category:$('#addCategory').value.trim(),tags:$('#addTags').value.trim(),rename:$('#addRename').value.trim(),auto_tmm:$('#addAutoTmm').value==='automatic',stopped:!$('#addStart').checked,add_to_top:$('#addTopQueue').checked,seed_mode:$('#addSeedMode').checked,sequential:$('#addSequential').checked,first_last:$('#addFirstLast').checked,stop_condition:$('#addStopCondition').value,content_layout:$('#addContentLayout').value,download_limit:Math.max(0,Number($('#addDownloadLimit').value||0))*1024,upload_limit:Math.max(0,Number($('#addUploadLimit').value||0))*1024,file_priorities:addFilePriorities()};try{if(addMetadataState.mode==='file'&&!addMetadataState.parsedFile){if(!file)throw new Error('Choose a .torrent file');let fd=new FormData();fd.append('server',state.server);fd.append('savepath',base.savepath);fd.append('downloadPath',base.download_path);fd.append('useDownloadPath',String(base.use_download_path));fd.append('category',base.category);fd.append('tags',base.tags);fd.append('rename',base.rename);fd.append('autoTMM',String(base.auto_tmm));fd.append('stopped',String(base.stopped));fd.append('addToTopOfQueue',String(base.add_to_top));fd.append('seedMode',String(base.seed_mode));fd.append('sequentialDownload',String(base.sequential));fd.append('firstLastPiecePrio',String(base.first_last));fd.append('stopCondition',base.stop_condition);fd.append('contentLayout',base.content_layout);fd.append('dlLimit',String(base.download_limit));fd.append('upLimit',String(base.upload_limit));fd.append('torrents',file);await api('/api/upload',{method:'POST',headers:{'X-CSRF-Token':state.csrf},body:fd})}else{if(!source)throw new Error('Enter a magnet link, torrent URL, or choose a .torrent file');await post('/api/action',{action:'add_magnet',urls:source,...base})}closeAddTorrent();$('#addForm').reset();toast('torrentAdded');setTimeout(refreshStatus,500)}catch(err){toast(err.message,'error')}}
